...
{
    for (int y = 0; y < g_infos.height; y++)
    {
        int index;
...
{
    if (pos.x < 0 || pos.x >= g_infos.width)
        return false;
    if (pos.y < 0 || pos.y >= g_infos.height)
        return false;
    *index = pos.y * g_infos.width + pos.x;
    return true;
}

bool index_to_pos(int index, t_pos *pos)
{
    *pos = npos(index % g_infos.width, index / g_infos.width);
    return index >= 0 && index < g_infos.len;
}

void print_board()
{
    for (int y = g_infos.height - 1; y >= 0; y--)
    {
        write(0, "|", 1);
        write(0, g_infos.board + (y * g_infos.width), g_infos.width);
        write(0, "|\n", 2);
    }
}
